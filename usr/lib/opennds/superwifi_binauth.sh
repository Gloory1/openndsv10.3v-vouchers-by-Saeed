...
update_voucher_accum() {
    ...
    local vouchers_file="${logdir}vouchers.txt"
    local debug_file="${logdir}debug.log"

    echo "------------------------------------------------------------------------------------------------" >> "$debug_file"
    echo "🤖 New operator | method : $method_type | raw url: $voucher_url" >> "$debug_file"

    voucher_code=$(echo -n "$voucher_url" | base64 -d )
    ...
    local season_used_kb=0
    if [ -n "$season_bytes_incoming" ]; then
        season_used_kb=$((season_bytes_incoming / 1024))
    fi
    ...
    local new_accum=$((old_accum + season_used_kb))
